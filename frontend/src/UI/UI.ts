...
export default class UI {
    ...
    static pushPlaceComponent(label: string, size: Vec2, offset: Vec2 = new Vec2(), settings?: ComponentSettings ) {

        if (!UI_I.setComponent(label)) {
            if(!settings)settings = new ComponentSettings()
            let comp = new Component(UI_I.getID(label), settings);
            UI_I.addComponent(comp);
        }
        UI_I.currentComponent.size = size
        UI_I.currentComponent.posOffset = offset
    }
