...
export default class LText extends LComponent
{
    ...
    private lines =[]
    constructor(id:number, label:string,text:string,multiLine:boolean,settings:LTextSettings) {
    ...
    layoutAbsolute() {
        ...
        this.textPos.y+= Math.floor(Utils.getCenterPlace(Font.charSize.y,settings.box.size.y))
    ...
    prepDraw() {
        ...
        if(this.multiLine) {
            ...
            for(let line of this.lines){
                UI_I.currentDrawBatch.textBatch.addLine(this.textPos, line, this.maxTextWidth, settings.textColor);
