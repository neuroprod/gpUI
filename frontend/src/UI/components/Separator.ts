...
export default class Separator extends Component {
    ...
    constructor(id: number, label: string,showLabel:boolean, settings: SeparatorSettings) {
        super(id, settings);
        this.label = label;
    ...
    layoutAbsolute() {
        ...
        if(this.showLabel)
        {
            ...
            this.maxLabelWidth = this.layoutRect.size.x - settings.labelPaddingLeft - 10;
            ...
            this.lineRectRight.size.x = this.layoutRect.size.x -  (this.lineRectRight.pos.x-this.layoutRect.pos.x) ;
            this.lineRectRight.pos.y+=this.layoutRect.size.y/2

            this.labelPos.y += Math.floor(Utils.getCenterPlace(Font.charSize.y, settings.box.size.y))
    ...
    prepDraw() {
        ...
        if(this.showLabel){
            ...
            UI_I.currentDrawBatch.fillBatch.addRect(this.lineRectRight,settings.lineColor)
        }
        UI_I.currentDrawBatch.fillBatch.addRect(this.lineRectLeft,settings.lineColor)
