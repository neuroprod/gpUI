...
export default class Event extends Component {
    ...
    constructor(id: number, label: string, text: string, isError: boolean, settings: EventSettings) {
        super(id, settings);
    ...
    layoutAbsolute() {
        ...
        this.labelPos.x += 10;
        this.labelPos.y += Math.floor(Utils.getCenterPlace(Font.charSize.y, this.layoutRect.size.y))
        this.maxLabelWidth = 80;


        this.textPos.copy(this.layoutRect.pos)
        this.textPos.x += 120 ;
        this.textPos.y += Math.floor(Utils.getCenterPlace(Font.charSize.y, this.layoutRect.size.y))
    ...
    prepDraw() {
        ...
        let settings = this.settings as EventSettings
        if (this.isError) {
            UI_I.currentDrawBatch.fillBatch.addRect(this.rightRect, settings.bgErrorColor);
        } else {
            UI_I.currentDrawBatch.fillBatch.addRect(this.rightRect, settings.bgColor);
        }
        UI_I.currentDrawBatch.fillBatch.addRect(this.leftRect, settings.bgColorRight);
        UI_I.currentDrawBatch.textBatch.addLine(this.labelPos, this.label, this.maxLabelWidth, settings.labelColor);
        UI_I.currentDrawBatch.textBatch.addLine(this.textPos, this.text, this.maxTextWidth, settings.textColor);
