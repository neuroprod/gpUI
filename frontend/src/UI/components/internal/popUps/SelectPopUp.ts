...
export default class SelectPopUp extends PopUp {
    ...
    constructor(id:number,callBack:(item:SelectItem)=>void,pos:Vec2,targetWidth=-1, items:Array<SelectItem> ,index=0,settings:SelectPopUpSettings) {
        super(id, settings);
        let itemSize =20;
        this.posOffset =pos;
        this.size.set(targetWidth,itemSize*items.length);
        this.items =items;
