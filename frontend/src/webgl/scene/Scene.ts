...
export default class Scene {
  ...
  private clearColor: ColorV = new ColorV(0.38, 0.35, 0.32, 1.0);
  private outlineColor: ColorV = new ColorV(0.22, 0.22, 0.22, 0.34);
  private selectColor: ColorV = new ColorV(1.0, 0.66, 0.0, 1.0);
  ...
  constructor(glMain: GL, preLoader: PreLoader) {
    ...
    this.cubes.push(
      new SceneCube(
        ...
        new Vector3(10.0, 0.01, 10.0),
        new Vector3(0.0, 0.0, 0.0),
        new ColorV(1.0, 1.0, 1.0, 1)
      )
    );
    this.cubes.push(
      new SceneCube(
        "Cube1",
        new Vector3(-2.26, 0.44, 1.04),
        new Vector3(1.0, 2.02, 2.0),
        new Vector3(0.0, -0.76, 0.0),
        new ColorV(0.72, 0.7, 0.69, 1.0)
      )
    );
    this.cubes.push(
      new SceneCube(
        "Cube2",
        new Vector3(-0.6, 1.84, 1.59),
        new Vector3(1.0, 2.97, 1.0),
        new Vector3(0.93, -0.3, 2.31),
        new ColorV(0.72, 0.7, 0.69, 1.0)
      )
    );
    this.cubes.push(
      new SceneCube(
        "Cube3",
        new Vector3(1.75, 1.57, 0.63),
        new Vector3(1.0, 5.78, 2.2),
        new Vector3(0.0, -0.89, 0.0),
        new ColorV(0.72, 0.7, 0.69, 1.0)
      )
    );
    this.cubes.push(
      new SceneCube(
        "Cube4",
        new Vector3(0.92, 0.56, 2.41),
        new Vector3(1.98, 2.87, 1.92),
        new Vector3(0.0, -0.66, 0.0),
        new ColorV(0.72, 0.7, 0.69, 1.0)
      )
    );
  ...
  update(viewPort: Vector4) {
    ...
    this.glMain.gl.clear(
      this.glMain.gl.COLOR_BUFFER_BIT | this.glMain.gl.DEPTH_BUFFER_BIT
    );
    this.programBlack.uniformMatrix4fv(
      "projection",
  ...
  draw() {
    ...
    this.programEdge.uniform3fv(
      "kernel",
      [
        1.0, 1.0, 0.5, -1.0, -1.0, 0.5, -1.0, 1.0, 0.5, 1.0, -1.0, 0.5, 1.0,
    ...
    let lSize = 1;
    let lSizeK = 1;
    this.programEdge.uniform2fv("screen", [
      this.viewPort.z * lSize,
      this.viewPort.w * lSize,
    ]);
    this.programEdge.uniform3fv("kernel", [
      lSizeK,
      lSizeK,
      0.5,
      -lSizeK,
