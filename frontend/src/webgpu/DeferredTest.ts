...
export default class DeferredTest {
  ...
  private boxColor1 = new ColorV(1.0, 1.0, 1.0, 1.0);
  private boxColor2 = new ColorV(0.72, 1.0, 0.0, 1.0);
  ...
  init() {
    ...
    this.material1 = new GBufferMaterial(
      this.device,
      "GbufferMaterial",
      this.shader
    );
    ...
    this.material2 = new GBufferMaterial(
      this.device,
      "GbufferMaterial",
      this.shader
    );
    ...
    this.material3 = new GBufferMaterial(
      this.device,
      "GbufferMaterial",
      this.shader
    );
    ...
    this.materialLight = new GBufferMaterial(
      this.device,
      "GbufferMaterial",
      this.shader
    );
    ...
    this.instanceShader = new InstanceColorShaderGBuffer(
      this.device,
      numInstances
    );
    this.instanceMaterial = new GBufferMaterial(
      this.device,
      "GbufferMaterial",
      this.instanceShader
    );
    ...
    for (let i = 0; i < positions.length; i++) {
      let pos = positions[i];
      let materialLight = new ForwardAddMaterial(
        this.device,
        "materialLight",
        this.lightShader,
      ...
      materialLight.setUniform(
        "lightPos",
        new Vector4(pos.x, pos.y, pos.z, 1.0)
      );
      materialLight.setTexture(
        "textureNormal",
      ...
      materialLight.setTexture(
        "textureAlbedo",
        this.gBufferPass.gBufferTextureAlbedo
      );
      let model = new Model(
        this.device,
        "gbuffermodel",
    ...
    this.materialAO = new ForwardMaterial(
      this.device,
      "materialAO",
      this.aoShader,
      this.aoPass.format,
    ...
    this.materialCombine = new ForwardMaterial(
      this.device,
      "materialCombine",
      this.combineShader,
    ...
    this.modelCombine = new Model(
      this.device,
      "combine",
      this.quad,
      this.materialCombine,
      false,
      this.camera
    );
    this.materialCombine.setTexture(
      "albedo",
      this.gBufferPass.gBufferTextureAlbedo
    );
    ...
    this.modelDofBlur2 = new Model(
      this.device,
      "combine",
      this.quad,
      this.materialDofBlur2,
      false,
      this.camera
    );
  ...
  update() {
    ...
    for (let m of this.modelsLight) {
      m.material.setUniform(
        "size",
        new Vector2(this.canvas.width, this.canvas.height)
      );
    }
    this.instanceMaterial.setUniform(
      "color",
      new Vector4(this.boxColor1.r, this.boxColor1.g, this.boxColor1.b, 0)
    );

    this.materialCombine.setUniform(
      "size",
      new Vector4(this.canvas.width, this.canvas.height, 0, 0)
    );
  ...
  prepDraw(context: GPUCanvasContext) {
    ...
    if (this.useAO) {
      ...
      this.materialCombine.setTexture(
        "ao",
        DefaultTexture.getWhite(this.device)
      );
    }
    this.materialCombine.setTexture(
      "albedo",
    ...
    if (this.currentView == Views.albedoGbuffer)
      this.materialFullScreen.setTexture(
        "texture1",
        this.gBufferPass.gBufferTextureAlbedo
      );
    else if (this.currentView == Views.normalGbuffer)
      this.materialFullScreen.setTexture(
        "texture1",
        this.gBufferPass.gBufferTextureNormal
      );
    else if (this.currentView == Views.positionGbuffer)
      this.materialFullScreen.setTexture(
        "texture1",
        this.gBufferPass.gBufferTexturePosition
      );
